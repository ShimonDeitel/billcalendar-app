...
final class Store: ObservableObject {
    @Published private(set) var items: [Bill] = []
    ...
    private let fileName = "billcalendar_items.json"
    ...
    private func load() {
        ...
        if let decoded = try? JSONDecoder().decode([Bill].self, from: data) {
            items = decoded
        }
    }
